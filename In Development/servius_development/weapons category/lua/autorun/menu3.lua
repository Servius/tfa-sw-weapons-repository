...
function SWV3_Spawn_SENT( player, EntityName, tr )
	...
	if ( sent ) then
		...
		local SpawnableEntities = list.Get( "SWVehiclesEU2" )
...
concommand.Add( "swv3_spawnsent", function( ply, cmd, args ) SWV_Spawn_SENT( ply, args[1] ) end ) 

function SWV3_CCGiveSWEP( player, command, arguments )

	if ( arguments[1] == nil ) then return end

	-- Make sure this is a SWEP
	local swept = list.Get( "SWVehiclesEU2.Weapons" );
	local swep;
...
concommand.Add( "swv3_giveswep", SWV3_CCGiveSWEP )

--[[---------------------------------------------------------
	-- Give a swep.. duh.
-----------------------------------------------------------]]
function SWV3_Spawn_Weapon( Player, wepname, tr )
...
concommand.Add( "swv3_spawnswep", function( ply, cmd, args ) SWV3_Spawn_Weapon( ply, args[1] ) end )    

end

if(CLIENT) then
spawnmenu.AddCreationTab( "Star Wars Weapons", function()
	local ctrl = vgui.Create( "SpawnmenuContentPanel" )
    ctrl:CallPopulateHook( "SWVehiclesEU2Tab" );
    return ctrl;
end, "icons16/other.png", 60 )

spawnmenu.AddContentType( "swvehicleeu2", function( container, obj )
    ...
    icon.DoClick = function()
        RunConsoleCommand( "swv3_spawnsent", obj.spawnname );
...
spawnmenu.AddContentType( "swweapon3", function( container, obj )
	...
	icon.DoClick = function()
		...
		RunConsoleCommand( "swv3_giveswep", obj.spawnname )
		surface.PlaySound( "ui/buttonclickrelease.wav" )
	...
	icon.DoMiddleClick = function()
		...
		RunConsoleCommand( "swv3_spawnswep", obj.spawnname )
		surface.PlaySound( "ui/buttonclickrelease.wav" )
...
hook.Add( "SWVehiclesEU2Tab", "AddEntityContent", function( pnlContent, tree, node )
    ...
    SpawnableEntities = list.Get( "SWVehiclesEU2.Weapons" )
	if ( SpawnableEntities ) then
		for k, v in pairs( SpawnableEntities ) do
			...
			v.SpawnName = k
 			if(v.Category == "Star Wars") then
                ...
                v.Category = string.gsub(v.Category, "%Star Wars Weapons: ", "") or "Other";
            end
			Categorised[ v.Category ] = Categorised[ v.Category ] or {}
			table.insert( Categorised[ v.Category ], v )
	...
	for CategoryName, v in SortedPairs( Categorised ) do
		...
		node.DoPopulate = function( self )
			...
			for k, ent in SortedPairsByMemberValue( v, "PrintName" ) do
                local enttype = "swvehicleeu2"
                if(CategoryName == "Weapons" or "Other") then
                    enttype = "weapon";
                end
