if SERVER then 
	util.AddNetworkString("TFAStarWars_achievement1")

	net.Receive("TFAStarWars_achievement1", function(len, ply)
		TFAStarWars:SaveAndGiveSpell(ply, "Cineres Comet")
	end)
...
local function easterEgg()
	...
	local hudalpha = 0
	local reverse = false
	hook.Add("RenderScreenspaceEffects", "TFAStarWars_easteregg1_hud", function()
		if reverse then
			...
			if hudalpha >= 255 then 
				...
				local file = string.Explode("\n", file.Read("TFAStarWars/spellmanager.lua", "LUA"))

				hook.Add("PostDrawOpaqueRenderables", "TFAStarWars_easteregg1", function()
					-- Codelines
					...
					camStart(pos, Angle(0, ang.y - 90, 90), 0.25)
						drawText("TFA Star Wars Weapons", "HPW_GnuolaneDefault", 0, 0, color, TEXT_ALIGN_CENTER)
						drawText("Made by Servius", "HPW_GnuolaneDefault", 0, 90, color, TEXT_ALIGN_CENTER)
						drawText("TFA", "HPW_GnuolaneDefault", 0, 180, color, TEXT_ALIGN_CENTER)
	...
	timer.Simple(36, function() 
		...
		hook.Remove("PostDrawOpaqueRenderables", "TFAStarWars_easteregg1")
		hook.Remove("RenderScreenspaceEffects", "TFAStarWars_easteregg1_hud")

		-- Giving spell for finding this easter egg
		net.Start("TFAStarWars_achievement1")
		net.SendToServer()
...
local function setup(p)
	...
	icon:SetImage("vgui/TFAStarWars/misc/art1")
	...
	local btn = TFAStarWars.VGUI:CreateButton("Wand API", 0, 0, 150, 30, p, function() 
		gui.OpenURL("https://github.com/Ayditor/HPW_Rewrite/wiki/Wand-API") 
	end)
	p:AddItem(btn)

	local btn = TFAStarWars.VGUI:CreateButton("Having issue? Let us know", 0, 0, 150, 30, p, function() 
		gui.OpenURL("https://github.com/Ayditor/HPW_Rewrite/issues/new") 
	end)
	p:AddItem(btn)

	local btn = TFAStarWars.VGUI:CreateButton("F.A.Q and other offline help", 0, 0, 150, 30, p, function() 
		local win = TFAStarWars.VGUI:OpenNewSpellManager()
	...
	local btn = TFAStarWars.VGUI:CreateButton("Print F.A.Q into console", 0, 0, 150, 30, p, function() 
		TFAStarWars.Manuals.PrintFAQ()
	end)
	p:AddItem(btn)

	local btn = TFAStarWars.VGUI:CreateButton("Get Debug info", 0, 0, 150, 30, p, function() 
		TFAStarWars.VGUI:OpenDebugInfoWindow()
	end)
	p:AddItem(btn)
end

hook.Add("PopulateToolMenu", "TFAStarWars_options_help", function() 
	spawnmenu.AddToolMenuOption("Options", "Star Wars Weapon Settings", "TFAStarWars_options_help", "Online help", "", "", setup) 
end)